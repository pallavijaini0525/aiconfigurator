...
from aiconfigurator.generator.xpu_utils import is_xpu_system
...
def load_custom_allreduce_data(custom_allreduce_file):
    ...
    def _path_matches_xpu(path: str) -> bool:
        # custom_allreduce paths include system segment (e.g. data/b60/...)
        # so we inspect path components and detect known XPU family names.
        for part in PurePosixPath(path).parts:
            if is_xpu_system(part):
                return True
        return False

    if isinstance(custom_allreduce_file, str):
        is_xpu = _path_matches_xpu(custom_allreduce_file)
    else:
        is_xpu = any(_path_matches_xpu(path) for path, _ in custom_allreduce_file)
